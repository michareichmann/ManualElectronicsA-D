...
\begin{itemize}
	...
	\begin{itemize}
        \item install the libraries from \href{http://wiki.seeedstudio.com/Grove-LCD_RGB_Backlight/}{http://wiki.seeedstudio.com/Grove-LCD_RGB_Backlight/}
        \item look at the example code on the above website, especially on how to include the libraries in your project and how to initialize it during the \ac{setup()} routine
        \item make sure the "3V3_VCC_5V" switch on the Grove shield is set to 5V, otherwise the display will not work correctly
	...
	\begin{itemize}
        \item read the description at \href{http://wiki.seeedstudio.com/Grove-Rotary_Angle_Sensor/}{http://wiki.seeedstudio.com/Grove-Rotary_Angle_Sensor/}
        \item read-out the sensor with \ac{analogRead()}
        \item to map the ADCs which range from 0 to 1023 to a reasonable temperature range (a, b), e.g. 20 to 40 degrees, the function \ac{map(degrees, 0, 1023, a, b)} can be used for convenience
 	\end{itemize} 
...
\begin{itemize}
	...
	\end{itemize}
	\item print the line in the terminal using \ac{print}
...
\end{itemize}
\b{Optional}: If you use Linux or Mac, try to do all of the above with a single line of bash with less than 100 characters. Look at \ac{read} and \ac{tee} commands.
%END
...
\begin{itemize}
	...
	\item setting the PWM duty-cycle can be done using the \textbf{analogWrite()} function
...
\begin{itemize}
	...
	\item check the Arduino reference of \textbf{analogRead()} for the maximum frequency. Since we also have to do other work in the \textbf{loop()} function, the frequency should be also much less than the maximum. Make a reasonable choice.
